...
There are several commands provided for generating clients, servers, and documentation. Each of these commands accept a `-schema` flag defaulting to `core.json`, see the `-h` help output for additional usage details.


### GO

- `srpc-go-types` generates Go type definitions
- `srpc-go-client` generates Go client
- `srpc-go-server` generates Go server


### Typescript (todo)

- `srpc-ts-client` generates TypeScript clients


## Schemas

Currently the schemas are loosely a superset of [JSON Schema](https://json-core.org/), however, this is a work in progress. See the [example schema](./examples/todo/core.json).
...
[![GoDoc](https://godoc.org/github.com/at-least/gen?status.svg)](https://godoc.org/github.com/at-least/gen)
